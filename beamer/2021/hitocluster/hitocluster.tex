...
\begin{frame}{Exploratives Data Mining}
\begin{itemize}
...
V_{out}(x)	&= \{v \in I| \langle v,p,x \rangle \in \textnormal{KB}\}\\
...
\end{align}
\[
\textnormal{NSWD}_{\lambda}(x,y) = \frac{\max(\log \fl{x}, \log \fl{y})- \log \fl{x,y}}{\log N - \min(\log \fl{x}, \log \fl{x})}
\]
\end{frame}

\begin{frame}{NSWD---Adaptations to HITO}
\begin{itemize}
\item Citations werden nur einmal benutzt und tauchen daher nie in $\f{x,y}$ auf
\item Properties können ignoriert werden, da keine Überschneidung im Wertebereich
\end{itemize}
